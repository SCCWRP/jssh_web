...
spyrsocc <- reactive({
  ...
  toplo <- toplo %>%  
    ...
    mutate(
      prcsit = 100 * nprs / nsites, 
      catsit = cut(prcsit, breaks = c(-Inf, 25, 50, 75, Inf), labels = c('rare', 'occasional', 'frequent', 'common')),
      catcol = factor(catsit, levels = c('rare', 'occasional', 'frequent', 'common'), labels = c("#E31A1C", "#FB9A99", "#B2DF8A", "#33A02C")), 
...
column(12, 
      ...
      column(width = 4, 
             selectInput('yrsub', 'Select from years only when present:', selected = FALSE, choices = c(TRUE, FALSE))
      ),
      column(2, NULL)
)
...
This map shows the occurrence of `r renderText({input$sptm})` at each site from `r renderText({input$yrs[1]})` to `r renderText({input$yrs[2]})`.  The colors indicate if a species was commonly (dark green), frequently (light green), occasionally (light red), or rarely (dark green) observed at a site.  Frequency occurrence was the sum of years for which a species was observed divided by the total number of years that a site was sampled.

...
library(scales)
library(leaflet)
library(ggord) # from fawda123 on github
library(vegan)
library(ggdendro)
library(dendextend)
library(ggrepel)
...
spdat <- fishdat %>% 
  ...
  mutate(commonname = if_else(is.na(commonname), species, commonname))

# create species choices for the species select items
spchoices <- spdat %>% 
  arrange(commonname) %>%
  pull(species) %>% unique

names(spchoices) <- spdat %>% 
  arrange(commonname) %>%
  pull(commonname) %>% unique
...
column(width = 4,
 selectInput('sptm2', 'Select a species:', spchoices, selected = 'Sthd')
)

# year slider
column(width = 4, 
  sliderInput("yrs2", label = 'Select year ranges to plot:',  
        min = 1981, max = max(unique(fishdat$Year)), 
        value = c(1994, max(unique(fishdat$Year))),
        sep = '', ticks = FALSE
      )
)
column(2, NULL)
...
column(12, 
      ...
      # select a species
      column(width = 4,
       selectInput('sp', 'Select a species:', spchoices, selected = 'Sthd')
      ), 
...
# select a species
column(width = 4,
 selectInput('sptm', 'Select a species:', spchoices, selected = 'Sthd')
)
...
column(12, 
       ...
       # select a year 
       column(width = 4, 
              selectInput('yrmlt1', 'Select a year for comparison:', sort(unique(mltdat$Year)), selected = max(unique(mltdat$Year)))
       ),
...
mltdatyr1 <- reactive({
  ...
  torm <- rowSums(toord[, !names(toord) %in% c('Watershed','SiteID')])
  toord <- toord[torm > 0, ] %>% filter(!is.na(SiteID))
  # toord <- toord %>%

...
soqbth <- soqfsh %>% 
  ...
  left_join(fishmtch, by = c('SiteID', 'mo')) %>% 
  mutate(mo = 'Sep') %>% 
  unite('mosite', mo, SiteFlow, sep = ' - ', remove = F) %>% 
  left_join(soqflo, by = c('mosite', 'date')) %>% 
  dplyr::select(SiteID, Site, date, `Size class`, density, flo) %>% 
...
Fish sample sites were matched to the flow records (September only) as follows:

```{r}
totab <- soqbth %>% 
  dplyr::select(SiteID, Site) %>% 
  unique
knitr::kable(totab, row.names = c(1:nrow(totab)))
```
...
renderPlot({
  ...
  p <- ggplot(soqbthyrs, aes(x = flo, y = density)) + 
    geom_point() + 
...
```{r, results = 'asis'}
renderUI({
  HTML(stargazer(soqtndmod(), type = 'html', column.labels = c('S1', 'S2'),

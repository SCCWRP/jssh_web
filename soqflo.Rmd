---
title: "Evaluation of flow trends at Soquel"
runtime: shiny
output: 
  html_document:
    toc: true
...
This page can be used to compare steelhead density estimates with modelled flow in the Soquel watershed.  The sections include an assessment of overall trends, summary trends, and plots by site.  The year slider can be used to choose the date range for all plots and trend analyses.  The size class selector shows which size class is evaluated for steelhead. All flow estimates are modelled for July and September at different locations on Soquel Creek that do not coincide with the fish sampling sites.  
...
```{r}
column(2, NULL)
column(4, selectInput('sitflo', 'Select flow site:', choices = unique(soqflo$mosite)))
column(6, NULL)
```
...
```{r}
column(2, NULL)
column(4, selectInput('sitfsh', 'Select fish sample site:', choices = unique(soqfsh$SiteID)))
column(6, NULL)
```

...
This section can be used to evaluate community changes over time by showing natural groupings between sites using species presence/absence data that are constrained by habitat variables.  Some questions to consider when evaluating the data are:
...
# select a year
column(width = 12, 
       selectInput('yrmlt', 'Select a year for comparison:', sort(unique(mltspdat$Year)), selected = 2017)
       )
```

### Species community data options
...
## Constrained ordinations {.tabset}
...
renderPlot({
  ...
  # }
  # 
  # pord <- ggord(ord(), grp_in = grpin, vec_ext = 1, col = colsub, 

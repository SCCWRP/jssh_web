...
column(12, 
       ...
       # select a year
       column(width = 4, 
         selectInput('yrmlt', 'Select a year for comparison:', sort(unique(mlthbdat$Year)), selected = max(unique(mlthbdat$Year)))
       ),
...
toord <- reactive({
  ...
  toordhb <- mlthbdat %>% 
    ...
    dplyr::select(SiteID, Watershed, habvar, habval) %>% 
      # in order to run spread we need unique values
      group_by(SiteID,Watershed,habvar) %>% 
      summarise(habval = mean(habval)) %>% 
      ungroup %>% 

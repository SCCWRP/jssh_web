...
mapview(stream, label = stream$STREAM_NM) +
  mapview(fishdat, zcol = 'Watershed', legend = T, layer.name = 'Watershed')
```
...
mapout <- reactive({
  ...
  out <- mapview(stream, label = stream$STREAM_NM) +
    mapview(toplo, zcol = 'Watershed', cex = input$cls, legend = T, layer.name = 'Watershed')
  
  return(out)
  
})

output$mapout <- renderLeaflet({mapout()@map})
```
```{r}
...
```{r, context = 'server'}
output$totabs2kn <- renderUI({
  HTML(knitr::kable(totabs2kn(), format = 'html') %>% 
    kable_styling(full_width = T, font_size = 14))
  })
```
```{r, results = 'asis'}
tableOutput('totabs2kn')
```
...
output$sitetrndmap <- renderLeaflet({mapoutst()@map})
```
```{r}
leafletOutput('sitetrndmap')
```

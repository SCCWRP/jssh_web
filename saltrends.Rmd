...
# globals
library(knitr)
opts_chunk$set(echo = F, message = F, warning = F, fig.path = './images/')
...
```{r densbox, fig.height = 7, fig.width = 6, fig.show = 'hide'}
toplo <- fishdat
...
```
```{r}
knitr::include_graphics('./images/densbox-1.png')
```
...
```{r denstile, fig.height = 4, fig.width = 7, fig.show = 'hide'}
...
```
```{r}
knitr::include_graphics('./images/denstile-1.png')
```

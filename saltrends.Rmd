...
output$totabs1 <- renderText({
  stargazer(totabs1()$res, type = 'html', column.labels = as.character(totabs1()$Watershed),
...
output$totabs1kn <- renderText({
  knitr::kable(totabs1kn(), format = 'html') %>% 
...
output$totabs2 <- renderText({
  stargazer(totabs2()$res, type = 'html', column.labels = as.character(totabs2()$Watershed),
            dep.var.labels = 'Density (fish/100ft)')
  })
...
output$totabs2kn <- renderText({
  knitr::kable(totabs2kn(), format = 'html') %>% 
    kable_styling(full_width = T, font_size = 14)
  })

...
These analyses can be used to evaluate changes in habitat over time, by watershed, habitat type, and habitat measurements. The first two sections show general changes by individual years and by watershed.  The third section shows more detailed changes for individual sites for selected periods of time. is a formal multivariate analysis showing ordination and clustering results of the communities for a selected year.  Changes over time can be assessed by comparing the results between years. 
...
habmap <- reactive({
  ...
  out <- mapview(stream, label = stream$STREAM_NM, homebutton = F) +
    mapview(habrct, zcol = 'Watershed', cex = 'habval', label = labs, legend = T, homebutton = F)

...
column(12, 
  ...
  # select year to show
  column(width = 4,
        selectInput('yr', 'Select a year:', selected = max(unique(habitat$Year)), choices = sort(unique(habitat$Year)))
  ),
  column(2, NULL)
)
column(12, 
...
column(12, 
  ...
  column(width = 4, 
    sliderInput("yrrng", label = 'Select year ranges to plot:',  
          min = 2001, max = max(unique(habitat$Year)), 
          value = c(2001, max(unique(habitat$Year))),
          sep = '', ticks = FALSE
        )
  ),

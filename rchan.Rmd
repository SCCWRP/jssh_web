...
This plot shows the distribution of the selected habitat variable as boxplots for each year, grouped and colored by the habitat selections.  Summary statistics are also shown based on the chosen comparison in the menu below.  The summary statistics can be used to evaluate differences __between habitat selections within the same year__ or __between years within the same habitat selection__.  Boxplots that share the same letter are not statistically different (at alpha = 0.05). The comparison of letters are only relevant for the selected summary statistics from the drop down menu. That is, letters can be only be compared within the same year if "between groups, within years" is selected, or letters can only be compared between years for the same group (habitat selection) if "between years, within groups" is selected.  Comparisons for boxplots without letters are not shown if the test could not be completed due to sample size.
